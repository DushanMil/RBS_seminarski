...
def go_to():
	...
	elif 'admin' in parsed_url.path or 'admin' in unquote(parsed_url.path):					#blacklist the endpoint 'admin'
		return 'Blocked: sneakyyy!'  
	
	# decode url
	decoded_path = unquote(unquote(parsed_url.path))
	...
	return urllib.request.urlopen(rebuilt_url).read()
